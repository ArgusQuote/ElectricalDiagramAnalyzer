...
# ---------- IMPORTS FROM REPO ----------
from VisualDetectionToolLibrary.PanelSearchToolV10 import PanelBoardSearch
from OcrLibrary.BreakerTableParserAPI import parse_image as _btp_parse_image
from OcrLibrary.PanelHeaderParserV4 import PanelParser as _HeaderParser
from OcrLibrary.BreakerTableAnalyzer3 import BreakerTableAnalyzer as _HeaderLocAnalyzer
...
def _warmup_ocr_once():
    try:
        ...
        # Warmup header OCR (EasyOCR) and analyzer OCR separately
        try:
            _hp = _HeaderParser(debug=False)
            if getattr(_hp, "reader", None):
                _hp.reader.readtext(dummy, detail=0)
                print(">>> Header OCR warmup complete")
        except Exception as e:
            print(f">>> Header OCR warmup skipped: {e}")

        try:
            _ana = _HeaderLocAnalyzer(debug=False)
            if getattr(_ana, "reader", None):
                _ana.reader.readtext(dummy, detail=0)
                print(">>> Analyzer OCR warmup complete")
        except Exception as e:
            print(f">>> Analyzer OCR warmup skipped: {e}")
...
def render_pdf_to_images(saved_pdf: Path, img_dir: Path, dpi: int = 400) -> list[str]:
    """
    Render a PDF to images for THIS job only.
    Uses a fresh PanelBoardSearch instance, so jobs don't contend on a global lock.
    """
    img_dir.mkdir(parents=True, exist_ok=True)
    print(f">>> rendering PDF → images: {saved_pdf} -> {img_dir} (dpi={dpi})")

    # Make a per-job finder (no shared state, no global lock)
    ...
    try:
        crops = local_finder.readPdf(str(saved_pdf))
    except Exception as e:
...
def _process_job(job_id: str):
    ...
    try:
        ...
        # Count breakers that would be skipped (mirrors your existing skip rules)
        def _count_would_skip_breakers(breakers: list[dict], panel_limit: int | None) -> int:
            ...
            for b in (breakers or []):
                ...
                if bad:
                    # count the whole aggregated group
                    total_bad += max(1, qty)
        ...
        def _merge_component_from_btp(result_dict: dict, src_img: str) -> dict:
            ...
            # ===== 4-strikes pre-check (use your same semantics for “skipped” breakers) =====
            # Ceiling for breaker amps: prefer the main breaker rating, else the panel rating
            panel_limit = None
            for cand in (main_amp, amperage):
                if isinstance(cand, int) and cand > 0:
                    panel_limit = cand
                    break

            # Count how many *individual* breakers would be skipped
            would_skip = _count_would_skip_breakers(det_brkrs, panel_limit)

            notes = []
            if would_skip >= 4:
                # Ship base panel only — no breakers passed to Rules Engine
                det_brkrs = []
        ...
        # ---------- IMAGE-LEVEL PARALLELISM (≤8 concurrent) ----------
        max_workers = max(1, min(4, os.cpu_count() or 1, total))
        print(f">>> parallel parsing with up to {max_workers} workers for {total} images")

        # Pre-allocate so we can preserve input order
        components = [None] * total

        # Use spawn to keep EasyOCR/PyTorch safe; each worker runs full A→P→H sequentially
...
def vm_submit_for_detection(media, ui_overrides=None, job_note=None, owner_email=None):
    ...
    img_dir = job_dir / "pdf_images"
    img_dir.mkdir(parents=True, exist_ok=True)
    images = render_pdf_to_images(saved_pdf, img_dir, dpi=400)

    # 5) Persist status (disk) with email only
    _status_write(
        job_dir,
        "queued",
        ...
        step="queued",
        noticed_ts_ms=noticed_ts_ms,
        owner_email=owner_email,  # <- canonical owner
        owner_id=owner_email,     # <- written for compatibility
        node_id=NODE_ID,
        canceled=False,
        progress=0.0
    )

    # 6) (Optional) upsert – keep as you had it, but store email too
    ...
    return {
        ...
        "noticed_ts_ms": noticed_ts_ms,
        "owner_email": owner_email,
        "owner_id": owner_email,  # compatibility
        "node_id": NODE_ID,
        "state": "queued"
    }
