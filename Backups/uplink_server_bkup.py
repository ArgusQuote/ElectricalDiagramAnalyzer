...
from OcrLibrary.PanelHeaderParserV3 import PanelParser
from OcrLibrary.PanelAnalysisAPI import PanelAnalysisAPI
...
FINDER = PanelBoardSearch(
    ...
    pad=4,
    use_ocr=False,  # Enable OCR (default: True)
...
def _process_job(job_id: str):
    ...
    try:
        ...
        for i, img_path in enumerate(imgs):
            print(f">>> parsing image {i+1}/{len(imgs)}: {img_path}")
            try:
                overlay_path = str(debug_dir / f"breaker_debug_{i+1:03d}.png")  # saved by BreakerTableExtractor if supported
                print(f">>> DEBUG: About to analyze {img_path} (overlay -> {overlay_path})")

                component = ANALYZER.analyze(img_path, debug_overlay_path=overlay_path)  # <-- NEW: merged header + table
                component = _normalize_component_for_none(component)
                ...
                print(f">>> parsed component: {component.get('name', 'unnamed')} (skipped: {component.get('_skipped', False)})")
