...
def _process_job(job_id: str):
    ...
    try:
        ...
        # Drop keys we explicitly pass again in subsequent _status_write calls
        _prev_carry = {
            k: v
        ...
        for start in range(0, total, BATCH_SIZE):
            ...
            batch_overlays = [str(debug_dir / f"breaker_debug_{start + j + 1:03d}.png") for j in range(len(batch_imgs))]

            print(f">>> parsing batch {start + 1}-{start + len(batch_imgs)} / {total}")
            try:
                batch_results = analyze_many_panels(batch_imgs, workers=workers, debug=False, overlays=batch_overlays)
            except Exception as e:
            ...
            done = min(total, start + len(batch_imgs))
            pct = 10.0 + (done / max(1, total)) * 80.0
...
@anvil.server.callable
def vm_submit_for_detection(media, ui_overrides=None, job_note=None, owner_id=None):
    """
    ...
    """
    if not owner_id:
        raise RuntimeError("owner_id required")

    original_name = getattr(media, "name", "uploaded.pdf")
    job_dir = _make_job_dir(job_note, original_name)
    job_id = job_dir.name
    print(f">>> vm_submit_for_detection: job_dir={job_dir}, owner_id={owner_id}")
    ...
    # 5) Persist status (disk) with owner_id
    _status_write(
        job_dir,
        "queued",
    ...
    # ----- NEW: verify and backfill owner_id on disk -----
    sp = _status_paths(job_dir)
    st = _json_read_or_none(sp["status"]) or {}
    stored_owner = str(st.get("owner_id") or "").strip()
    caller_owner = str(owner_id or "").strip()
    if not stored_owner:
        st["owner_id"] = caller_owner
        _status_write(job_dir, st.get("state", "queued"),
                    **{k: v for k, v in st.items() if k not in ("state", "ts")})
        print(f">>> backfilled owner_id for {job_id}: '{caller_owner}'")
    else:
        print(f">>> owner_id present for {job_id}: '{stored_owner}'")
...
@anvil.server.callable
def vm_get_job_status(job_id: str, owner_id: str) -> dict:
    job_dir = BASE_JOBS_DIR / job_id
    sp = _status_paths(job_dir)
    ...
    stored = str(st.get("owner_id") or "").strip()

    if not stored:
        # legacy/missing owner_id: repair and continue
        st["owner_id"] = caller
        _status_write(job_dir, st.get("state", "unknown"),
                      **{k: v for k, v in st.items() if k not in ("state", "ts")})
        stored = caller
        print(f">>> vm_get_job_status backfilled owner_id for {job_id}")
