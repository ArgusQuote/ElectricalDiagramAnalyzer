...
# ---------- IMPORTS FROM REPO ----------
from VisualDetectionToolLibrary.PanelSearchToolV9 import PanelBoardSearch
...
FINDER = PanelBoardSearch(
    ...
    dpi=400,
    min_whitespace_area_fr=0.01,  # big green fields
    margin_shave_px=6,
    min_void_area_fr=0.004,       # filter tiny junk
    min_void_w_px=90,
    min_void_h_px=90,
    max_void_area_fr=0.50,
    pad=6,
    debug=False,                  # overlays only if True
    verbose=True,
    save_masked_shape_crop=False,  # set True to also save exact-shape crops
...
def vm_get_job_status(job_id: str, owner_id: str) -> dict:
    """Status primarily from disk; returns result when done. Enforces ownership."""
    job_dir = BASE_JOBS_DIR / job_id
    sp = _status_paths(job_dir)
