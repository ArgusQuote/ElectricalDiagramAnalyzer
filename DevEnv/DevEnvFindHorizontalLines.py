...
# ===================== USER VARIABLES =====================
SOURCE_IMG = "~/Documents/Diagrams/CaseStudy_VectorCrop_Run1/ELECTRICAL SET (Mark Up)_electrical_filtered_page002_panel01.png"              # <- set me
OUTPUT_DIR = "~/Documents/Diagrams/LineCounting2"                  # <- set me
RUN_LEN     = 20                                    # consecutive next_gap count
...
def detect_horizontal_lines(
    ...
    Returns a list of dicts:
      {y_center, y_top, y_bottom, x_left, x_right, width, height}
    """
    H, W = img_bgr.shape[:2]
    ...
    klen = max(5, int(round(W * horiz_kernel_frac)))
    kh = cv2.getStructuringElement(cv2.MORPH_RECT, (klen, 1))
    horiz = cv2.morphologyEx(thr, cv2.MORPH_OPEN, kh)

    # clean small junk while keeping hairlines
    horiz = cv2.dilate(horiz, None, iterations=1)
    horiz = cv2.erode(horiz, None, iterations=1)
...
def main():
    in_path = os.path.expanduser(SOURCE_IMG)
    out_dir = Path(os.path.expanduser(OUTPUT_DIR))
    out_dir.mkdir(parents=True, exist_ok=True)

    img = load_bgr(in_path)
    ...
    overlay_path = out_dir / "overlay_horizontal_lines.png"
    cv2.imwrite(str(overlay_path), overlay)

    # full geometry + gaps
    lines_json_path = out_dir / "lines.json"
    ...
    meas_json_path = out_dir / "line_measurements.json"
    ...
    # summary
    summary = {
        "input_image": str(Path(in_path).resolve()),
    ...
    print(f"uniform_run_found={has_uniform} | lines={len(lines)}")
    print(f"Wrote:\n  {overlay_path}\n  {lines_json_path}\n  {meas_json_path}\n  {summary_json_path}")
