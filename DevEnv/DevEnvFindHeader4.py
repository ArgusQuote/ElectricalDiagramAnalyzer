...
FOOTER_PREFER_ABOVE = True     # usually the footer rule is just above the text
...
def ocr_numeric_tokens_in_crop(crop_gray: np.ndarray, ctx: OCRCtx):
    ...
    if not out:
        return None, []
    # pick bottom-most numeric token = footer text
    out.sort(key=lambda t: t["y2"])
    return out[-1], out
        ...
        if (hdr_line_y is None) or (sob_line_y is None) or (cct_tok is None):
            ...
            if xL is None or xR is None:
                ...
                if last_num_token is None:
                    ...
                    # ---------- Find footer line on horizontal mask near footer text ----------
                    ...
                    if ftr_line_y is None:
                        ftr_line_y = scan_white_band_near(
                            ftr_text_y, hmask_thick, W,
                            prefer_above=FOOTER_PREFER_ABOVE,
                            search_px=FOOTER_SEARCH_PX,
                            min_run_frac=MIN_WIDTH_FRAC
                        )
