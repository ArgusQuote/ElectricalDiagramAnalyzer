...
# ---- horizontal lines from mask (for header + start-of-breakers) ----
...
# ---------------- column bracketing (verticals below SoB) ----------------
def vert_mask_strong_roi(bw_inv: np.ndarray, x1:int, y1:int, x2:int, y2:int):
    """Strong vertical detector but constrained to ROI (below start-of-breakers)."""
    roi = bw_inv[y1:y2, x1:x2]
    if roi.size == 0:
        return np.zeros_like(bw_inv)
    H, W = roi.shape
    # multi-scale vertical open + close
...
def find_column_bounds(vmask: np.ndarray, x_ref: int, y_top: int, H: int, search_margin_px: int = 400):
    """
    Given a vertical mask and a reference x (from CCT token), find nearest vertical lines
    left and right (within a horizontal window). Returns (xL, xR) or (None, None).
    """
    x_lo = max(0, x_ref - search_margin_px)
    x_hi = min(vmask.shape[1] - 1, x_ref + search_margin_px)
    colsum = vmask[y_top:H, :].sum(axis=0)  # only look below y_top
    xs = np.where(colsum > 0)[0]
    xs = xs[(xs >= x_lo) & (xs <= x_hi)]
    if xs.size == 0:
        return None, None

    # group xs into line columns by proximity
...
def degrid_crop(gray_crop: np.ndarray) -> np.ndarray:
    """
    Remove long vertical/horizontal lines in the crop via inpaint.
    """
...
# ---------------- drawing ----------------
def draw_header_overlay_full(gray, hdr_text_y, hdr_line_y, sob_line_y):
...
def main():
    ...
    for img_path in files:
        ...
        sob_line_y = None
        if hdr_line_y is not None:
            # choose next *below* header line, or scan for wide white band just below
            sob_line_y = pick_nearest_line(
                lines, hdr_line_y + 2,
                prefer_above=SOB_PREFER_ABOVE,  # i.e., below
                gap_min=1,
        ...
        # Always write the header overlay image (full page)
        vis_header = draw_header_overlay_full(gray, hdr_text_y, hdr_line_y, sob_line_y)
        cv2.imwrite(os.path.join(OUTPUT_DIR, f"{base}_HEADER_OVERLAY.png"), vis_header)

        # ---------- Column crop (source image) ----------
        # 1) get first CKT/CCT token (top-most) in left band
        cct_tok = first_cct_token(gray, ctx, right_frac=CCT_COL_RIGHT_FRAC)

        # 2) require header and start-of-breakers and a CCT token to proceed
        if (hdr_line_y is None) or (sob_line_y is None) or (cct_tok is None):
            print(f"[SKIP] {base}: missing pieces "
                  f"(hdr_line={hdr_line_y} sob={sob_line_y} cct_tok={'yes' if cct_tok else 'no'})")
            continue

        # 3) build vertical mask only below start-of-breakers, then bracket column around token center
        ...
        if xL is None or xR is None:
            print(f"[SKIP] {base}: could not bracket CKT column (xL={xL}, xR={xR}).")
            continue

        # 4) crop from SOURCE image (not masks), below start-of-breakers
        crop = bgr[y_top:H, xL:xR].copy()
        crop_gray = cv2.cvtColor(crop, cv2.COLOR_BGR2GRAY)

        # 5) remove grids in the crop, and save ONLY the gridless crop
        ...
        print(f"[OK] {base}: header_text_y={hdr_text_y} header_line={hdr_line_y} "
              f"start_of_breakers={sob_line_y} | column=({xL},{xR}) → {out_col}")
