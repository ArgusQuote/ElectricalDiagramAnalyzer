...
class PanelBoardSearch:
    ...
    def _process_page(self, img_bgr: np.ndarray, base_name: str, page_idx: int) -> list[str]:
        ...
        # Optional green overlay for kept whitespace (debug only)
        if self.debug:
            overlay_green = self._draw_green_overlay(img_bgr, labels, stats, keep_ids, alpha=0.45)

        # Magenta perimeter overlay (only saved if we actually draw something)
        ...
        if hier is not None and len(hier) > 0:
            ...
            for idx, cnt in enumerate(contours):
                ...
                # Draw magenta perimeter for accepted voids
        ...
        # Save magenta overlay ONLY if something was drawn
        if drew_magenta:
