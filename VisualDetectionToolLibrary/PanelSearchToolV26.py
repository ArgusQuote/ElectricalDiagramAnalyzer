...
class PanelBoardSearch:
    ...
    def readPdf(
        ...
        """
        Detect panel-board voids on each PDF page, export vector-PDF clips
        and high-DPI PNG crops, and return the list of PNG paths.

        Normal production mode:
            readPdf(pdf_path)

            Processes the full PDF exactly as before.

        Incremental compatibility mode:
            readPdf(
                pdf_path,
                candidate_callback=my_callback,
            )

            Each candidate is validated and cleaned individually, then passed
            to candidate_callback(candidate_path).

            The callback should return:
                False -> reject this item and continue searching
                True  -> accept this item and stop searching immediately
        """
        ...
        if candidate_callback is not None and not callable(candidate_callback):
            raise TypeError(
                "candidate_callback must be callable or None."
            )
        ...
        # Used only by incremental compatibility mode.
        # Normal production mode leaves this False.
        ...
        for pidx in range(len(doc)):
            ...
            for clip in candidates:
                ...
                write_ok = cv2.imwrite(str(png_path), png)

                if not write_ok:
                    if self.verbose:
                        print(f"[WARN] Failed to write candidate crop: {png_path}")
                    continue
                ...
                # collect every raw candidate and run the normal post-pass
                # after the full PDF has been searched.
                if candidate_callback is None:
                    all_pngs.append(png_path_str)

                # ---------------------------------------------------------
                # INCREMENTAL COMPATIBILITY MODE
                # ---------------------------------------------------------
                else:
                    # Run the existing horizontal-line/table validation on only
                    # this candidate.
                    valid_paths, invalid_paths = self._save_horizontal_line_masks(
                        [png_path_str]
                    )

                    # This item looked panel-like geometrically but failed the
                    # existing table-spacing check. Do not send it to OCR.
                    if not valid_paths:
                        if self.verbose:
                            print(
                                f"[INFO] Incremental candidate rejected by "
                                f"table validation: {png_path_str}"
                            )
                        continue

                    ready_paths = valid_paths

                    # Run the existing one-table-per-image cleanup before
                    # sending the candidate into BreakerTablePipeline.
                    if self.enforce_one_box:
                        ready_paths = self._enforce_one_box_on_paths(
                            ready_paths
                        )

                    # A raw candidate can occasionally split into multiple tables.
                    # Send those cleaned tables to the callback one at a time.
                    for ready_path in ready_paths:
                        if self.verbose:
                            print(
                                f"[INFO] Sending incremental candidate "
                                f"to callback: {ready_path}"
                            )

                        should_stop = bool(
                            candidate_callback(ready_path)
                        )

                        if should_stop:
                            # In callback mode, only return the accepted candidate.
                            all_pngs.append(ready_path)
                            stop_requested = True

                            if self.verbose:
                                print(
                                    f"[INFO] Candidate accepted by callback. "
                                    f"Stopping panel search: {ready_path}"
                                )
            ...
            # candidate_callback returned True, so do not inspect any more
            # candidates or PDF pages.
            if stop_requested:
                if self.verbose:
                    print(
                        f"[INFO] Incremental panel search stopped "
                        f"after page {pidx+1}."
                    )
                break
        ...
        # Normal production mode keeps the existing full-list post-processing.
        # Incremental compatibility mode has already run these exact checks
        # candidate-by-candidate before invoking candidate_callback.
        if candidate_callback is None:

            # --- post-pass: FIRST run horizontal-line spacing / validity ---
            if all_pngs:
                valid_pngs, invalid_pngs = self._save_horizontal_line_masks(
                    all_pngs
                )
                all_pngs = valid_pngs

            # --- then enforce one table per PNG ---
            if self.enforce_one_box and all_pngs:
                all_pngs = self._enforce_one_box_on_paths(
                    all_pngs
                )

        # Normal production mode keeps the existing full-document raster pass.
        # Compatibility mode does not need raster outputs or completed overlays,
        # and running this would unnecessarily reopen and scan the whole PDF
        # after the accepted panel was already found.
        if candidate_callback is None:
            self._extract_rasters_and_augment_magenta(
                pdf_path_str,
                page_void_boxes,
            )
        ...
        if self.verbose:
            ...
            if candidate_callback is None:
                ...
                print(
                    f"      Accepted candidate count → "
                    f"{len(all_pngs)}"
                )
                print(
                    f"      Search stopped early → "
                    f"{stop_requested}"
                )
