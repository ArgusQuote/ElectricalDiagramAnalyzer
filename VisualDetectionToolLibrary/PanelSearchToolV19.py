...
import fitz  # PyMuPDF
import json
import shutil   # <-- needed for invalid table moves
...
class PanelBoardSearch:
    ...
    NEW:
      - Detect embedded raster images first, crop them to PNGs into
        <output_dir>/raster_images, and keep their regions.
      - When finding table 'voids', ignore regions that overlap rasters.
    ...
    def readPdf(self, pdf_path: str) -> list[str]:
        ...
        for pidx in range(len(doc)):
            ...
            # ---------- 1) Detect & crop raster images for this page ----------
            ...
            try:
                ...
                if self.verbose:
                    print(f"[WARN] {base} page {pidx+1}: get_images() for rasters failed: {e}")
                page_images = []
            ...
            for img_info in page_images:
                ...
                try:
                    ...
                    if self.verbose:
                        print(f"[WARN] {base} page {pidx+1}: get_image_rects({xref}) failed: {e}")
                    continue

                for r in rects:
                    rect_obj = fitz.Rect(r)
                    raster_rects.append(rect_obj)

                    # Render a high-DPI PNG crop for the raster region
                    raster_idx += 1
                    try:
                        pix_r = page.get_pixmap(
                            matrix=rend_mat,
                            clip=rect_obj,
                            alpha=False,
                            colorspace=cs,
                        ...
                        if self.verbose:
                            print(f"[WARN] {base} page {pidx+1}: raster pixmap failed: {e}")
                        continue

                    raster_img = self._pix_to_bgr(pix_r)
                    if self.downsample_max_w and raster_img.shape[1] > self.downsample_max_w:
                        scale = self.downsample_max_w / float(raster_img.shape[1])
                        new_wh = (
                            self.downsample_max_w,
                            max(1, int(round(raster_img.shape[0] * scale))),
                        )
                        raster_img = cv2.resize(raster_img, new_wh, interpolation=cv2.INTER_AREA)

                    r_png_path = (
                        self.raster_dir /
                        f"{base}_page{pidx+1:03d}_raster{raster_idx:02d}.png"
                    )
                    cv2.imwrite(str(r_png_path), raster_img)
                    if self.verbose:
                        print(f"[INFO] Page {pidx+1}: saved raster {raster_idx} → {r_png_path}")

            # ---------- 2) Detection bitmap (AA=0) for whitespace / tables ----------
            ...
            if hier is not None and len(hier) > 0:
                ...
                for ci, cnt in enumerate(contours):
                    ...
                    # Skip candidates that substantially overlap a raster image
                    if self._clip_overlaps_any_raster(clip, raster_rects, frac_thresh=0.2):
                        if self.verbose:
                            print(f"[INFO] Page {pidx+1}: void overlaps raster; skipping as table.")
                        continue
            ...
            if not candidates:
                for (x0, y0, x1, y1) in self._grid_proposals_fallback(det_bgr):
                    ...
                    if self._clip_overlaps_any_raster(clip, raster_rects, frac_thresh=0.2):
                        if self.verbose:
                            print(f"[INFO] Page {pidx+1}: fallback void overlaps raster; skipping.")
                        continue
    ...
    # --------- Raster overlap helpers (new) ----------
    @staticmethod
    def _rect_overlap_fraction(a: fitz.Rect, b: fitz.Rect) -> float:
        """
        Return fraction of rect `a` covered by intersection with `b`.
        """
        inter = a & b
        if inter.is_empty:
            return 0.0
        a_area = max(a.width, 0) * max(a.height, 0)
        if a_area <= 0:
            return 0.0
    ...
    @staticmethod
    def _clip_overlaps_any_raster(
        clip: fitz.Rect,
        raster_rects: list[fitz.Rect],
        frac_thresh: float = 0.2,
    ) -> bool:
        """
        True if `clip` overlaps any raster rect by at least `frac_thresh`
        of the clip's area.
        """
        if not raster_rects:
            return False
        for r in raster_rects:
            if PanelBoardSearch._rect_overlap_fraction(clip, r) >= frac_thresh:
                return True
        return False
    ...
    def _save_horizontal_line_masks(self, image_paths: list[str]) -> tuple[list[str], list[str]]:
        ...
        Any crop whose valid_table == False is moved into:
            <output_dir>/invalid_tables/

        Returns:
            (valid_paths, invalid_paths)
            - valid_paths: PNG paths that remain in their original location
            - invalid_paths: PNG paths after being moved into invalid_tables
        """
        ...
        for p in image_paths:
            ...
            if img is None:
                ...
                # if we can't read it, treat as invalid but don't move (leave as-is)
            ...
            if not valid_table:
                ...
                try:
                    ...
                    # if move fails, keep it in place
                    invalid_paths.append(p)
            else:
                valid_paths.append(p)

            # --- Debug artifacts only if debug=True ---
            if not self.debug:
                continue
