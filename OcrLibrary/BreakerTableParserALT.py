...
class BreakerTableParser:
    ...
    def _infer_poles_from_trip_boxes(
        self,
        trip_boxes: List[Tuple[int,int,int,int,str]],
        rows_boxes: List[Tuple[int,int,int,int]],
    ) -> List[Tuple[int, Optional[int]]]:
        """
        Use tokens per row from the TRIP column to infer poles.

        Rules (per your example):
        • amperage at row r:
            - if row r+1 contains '2P'            → 2P breaker at amps[r] (consume r & r+1)
            - if row r+2 contains '3P' AND row r+1 is blank or '/' or '-' 
                                                → 3P breaker at amps[r] (consume r..r+2)
            - else                               → 1P breaker at amps[r] (consume r)
        • rows that only contain '2P','3P','/','-' are continuations/separators.
        """
        if not rows_boxes:
            return []

        # Map each token to the nearest row index
        def row_for(yc: int) -> Optional[int]:
            best = None; best_d = 1e9
            for i, (y1, y2, _x1, _x2) in enumerate(rows_boxes):
                if y1 <= yc <= y2:
                    return i
                c = 0.5 * (y1 + y2)
                d = abs(float(yc) - c)
        ...
        for (_x1, y1, _x2, y2, tok) in trip_boxes:
            r = row_for(int((y1 + y2) / 2))
            if r is not None and 0 <= r < n and tok:
                row_tokens[r].append(str(tok).upper().strip())

        # Derive per-row amperage and flags
        amps: List[Optional[int]] = [None] * n
        has_2p = [False] * n
        has_3p = [False] * n
        is_sep = [False] * n  # row has only '/' or '-' (or nothing)
        ...
        for i, toks in enumerate(row_tokens):
            ...
            any_amp = False
            for t in toks:
                if re.fullmatch(r"[23]P", t):
                    if t == "2P": has_2p[i] = True
                    if t == "3P": has_3p[i] = True
                elif t in ("/", "-"):
                    # could be alone or with others; we'll mark sep only if no amps/tags
                    pass
                else:
                    m = re.search(r"\d{1,4}", t)
                    if m:
                        try:
                            amps[i] = int(m.group(0))
                            any_amp = True
                        except Exception:
                            pass
            # separator row if nothing but '/', '-' tokens
            if not any_amp and not has_2p[i] and not has_3p[i]:
                if any(t in ("/", "-") for t in toks) or len(toks) == 0:
                    is_sep[i] = True

        # Walk rows and emit breakers
        ...
        while r < n:
            ...
            # 2P case: next row explicitly '2P'
            if (r + 1) < n and has_2p[r + 1]:
                out.append((2, a))
                r += 2
                continue

            # 3P case: row+2 is '3P' and row+1 is blank or sep ('/' or '-')
            if (r + 2) < n and has_3p[r + 2] and (is_sep[r + 1] or amps[r + 1] is None):
                out.append((3, a))
                r += 3
                continue
    ...
    def parse_from_analyzer(self, analyzer_payload: Dict) -> Dict:
        ...
        # Build detected_breakers (trips-only friendly)
        def _to_int(s):
        ...
        if has_poles:
            ...
            # Use the TRIP boxes (with y-coords), mapped to rows, then run-length encode.
            L_boxes = all_finds.get("l_trip_boxes", []) or []
            R_boxes = all_finds.get("r_trip_boxes", []) or []

            # rows_boxes already computed earlier from centers/header/footer
            left_runs  = self._infer_poles_from_trip_boxes(L_boxes, rows_boxes)
            right_runs = self._infer_poles_from_trip_boxes(R_boxes, rows_boxes)

            counts = {}
            for poles, amps in (left_runs + right_runs):
                if amps is None or poles is None:
                    continue
                poles = int(max(1, min(3, int(poles))))  # clamp to 1..3
                key = (poles, int(amps))
                counts[key] = counts.get(key, 0) + 1
    ...
    def _columns_from_header_map(self, header_map: Dict[str, List[dict]], page_width: int,
        ...
        # Prefer snapping to the actual header pick; only accept if close to band center.
    ...
    def _render_header_overlay(self, gray: np.ndarray, y_band: Tuple[int,int],
        ...
        YELLOW = (0,255,255); RED=(0,0,255); BLUE=(255,0,0); BLACK=(0,0,0); WHITE=(255,255,255)
    ...
    def _save_page_overlay(self, gray: np.ndarray, header_y: int, footer_y: int,
        ...
        for role, color in [("trip",(0,0,255)), ("description",(255,0,0)), ("ckt",(0,0,0))]:
