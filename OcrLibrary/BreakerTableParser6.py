...
_MIN_OCR_CONF = 0.20
_MIN_COMBO_CONF = 0.40  # breakers should be very clear
...
# Restrict OCR charset inside header band to typical header glyphs
_HDR_OCR_ALLOWLIST = "ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789() -/"
...
def _nms_merge(dets, iou_thresh=0.5):
    """
    dets: list of dicts with keys: text, conf, x1,y1,x2,y2
    Returns a pruned list: keep highest-conf per-overlap and unify near-duplicates.
    """
    # Normalize text up-front (uppercase; trim)
    for d in dets:
    ...
    for d in dets:
        drop = False
        for k in kept:
            # If boxes overlap a lot OR same centerline and nearly same x-span, treat as dup
            if _iou(d, k) >= iou_thresh:
                drop = True
                break
            cy_d = 0.5 * (d["y1"] + d["y2"])
            cy_k = 0.5 * (k["y1"] + k["y2"])
            if abs(cy_d - cy_k) <= 3 and abs(d["x1"] - k["x1"]) <= 6 and abs(d["x2"] - k["x2"]) <= 6:
                drop = True
                break
        if not drop:
            kept.append(d)
...
def _dedup_tokens(tokens: List[Dict], iou_thr: float = 0.80) -> List[Dict]:
    """
    ...
    for t in tokens:
        tt = dict(t)
        tt['text_norm'] = _norm_header_text(tt['text'])
        normed.append(tt)
    ...
    for i, ti in enumerate(normed):
        ...
        for j in range(i+1, len(normed)):
            ...
            if ti['text_norm'] != tj['text_norm']:
                continue
            if _bbox_iou(ti['box'], tj['box']) >= iou_thr:
                suppressed[j] = True
    return kept

def _post_label_join(words: List[str]) -> str:
    """
...
def group_header_tokens_plumbline(tokens: List[Dict], eps: int = 3) -> List[Dict]:
    """
    ...
    for t in toks:
        ...
        t['text_norm'] = _norm_header_text(t.get('text', ''))
...
def _normalize_header_tokens(tokens: list[str]) -> str:
    """
    Normalize stacked/wrapped header words into a canonical phrase.
    Rules:
      - Collapse punctuation variants: 'AMP .' / 'AMP.' -> 'AMP'
      - Join CKT + NO -> 'CKT NO'
      - Join BRKR + NOTES -> 'BRKR NOTES'
      - Join BRKR + AMP (+ ,) + POLES -> 'BRKR AMP, POLES'
      - Keep DESCRIPTION as-is
      - Deduplicate and keep a sensible order for known combos
    """
    toks = []
    for t in tokens:
        ...
        s = re.sub(r"\s+", " ", s)
        s = re.sub(r"[^\w ]+", "", s)  # drop stray punctuation except space
        if s:
            toks.append(s)
    ...
    has = lambda w: any(w == x for x in toks)

    # Known canonical phrases (priority)
    if has("DESCRIPTION"):
        return "DESCRIPTION"
    if has("CKT") and has("NO"):
        return "CKT NO"
    if has("BRKR") and has("NOTES"):
        return "BRKR NOTES"
    if has("BRKR") and has("AMP") and has("POLES"):
        return "BRKR AMP, POLES"

    # Fallback: if we only have one or two meaningful tokens, join them
    # (e.g., 'CKT', 'NO' -> handled above; 'BRKR', 'AMP' -> 'BRKR AMP')
    if len(toks) <= 3:
        return " ".join(toks)

    # Last resort: keep the most informative subset in a stable order
    ORDER = ["BRKR", "AMP", "POLES", "CKT", "NO", "DESCRIPTION", "NOTES"]
    ordered = [w for w in ORDER if w in toks]
    if ordered:
        # special: if AMP and POLES both present and BRKR absent, make 'AMP, POLES'
        if "AMP" in ordered and "POLES" in ordered and "BRKR" not in ordered:
            return "AMP, POLES"
        return " ".join(ordered)
    return " ".join(toks)

def _collapse_band_tokens_to_label(band_obj: dict) -> str:
    """
    Given a band object from _group_text_by_columns (with 'tokens'),
    join vertically-stacked tokens and normalize to one label.
    """
    toks = [t.get("text","") for t in (band_obj.get("tokens") or [])]
    return _normalize_header_tokens(toks)

def _crop_header_band(img_bgr: np.ndarray, header_y: int, band_h: int = _HDR_BAND_H, pad: int = _HDR_PAD_PX):
    h, w = img_bgr.shape[:2]
...
def _components_to_column_rois(bin_img: np.ndarray):
    inv = 255 - bin_img
...
def _ocr_header_columns(reader, bin_band: np.ndarray):
    rois = _components_to_column_rois(bin_band)
    cols = []
...
def detect_header_columns(reader, img_bgr: np.ndarray, header_y: int):
    band = _crop_header_band(img_bgr, header_y)
    cols = _ocr_header_columns(reader, band)
    col_map = {}
    for c in cols:
...
def classify_header_columns_basic(header_groups_lr: List[dict], dump: List[dict], page_width: Optional[int] = None) -> dict:
    ...
    # --- fuzzy helpers (edit distance ≤ 1) for OCR typos like 'APS' -> 'AMPS' ---
    def _lev1(a: str, b: str) -> bool:
        ...
        while i < len(a) and j < len(b):
            if a[i] == b[j]:
                ...
                if edits > 1:
                    return False
                # try insert/skip in longer string
                if len(a) != len(b):
                    j += 1
                else:
                    i += 1; j += 1
        # tail of longer string counts as one edit at most
        edits += (len(b) - j)
        return edits <= 1

    def _has_fuzzy(word_list, targets):
        for w in word_list:
            uw = w.upper()
            for t in targets:
                if _lev1(uw, t):
    ...
    for g in (header_groups_lr or []):
        ...
        norm_lab  = _norm(label)
        # regex flags on normalized label
        t_trip_rx  = bool(ROLE_RX["trip"].search(label))
        t_poles_rx = bool(ROLE_RX["poles"].search(label))

        # --- fuzzy flags (split into alpha tokens) ---
        words_alpha = re.findall(r"[A-Z]+", norm_lab)
        has_fuzzy_amp   = _has_fuzzy(words_alpha, ["AMP", "AMPS", "TRIP"])
        has_fuzzy_poles = _has_fuzzy(words_alpha, ["POLE", "POLES"])
        t_trip  = t_trip_rx  or has_fuzzy_amp
        t_poles = t_poles_rx or has_fuzzy_poles
        ...
        # combo if both roles present OR any known combined phrasing OR fuzzy AMP+POLES
        is_combo = (
            (t_trip and t_poles)
            or ("AMP, POLES" in norm_lab)
            or ("TRIP POLE" in norm_lab)
            or ("BRKR AMP POLES" in norm_lab)
        )
...
def split_multi_role(d: dict) -> list:
    """
...
class BreakerTableParser:
    ...
    def _group_text_by_xcenters(self, items, y_band, *,
        ...
        for it in (items or []):
            try:
                ...
                expanded = []
                for s in split_by_words(it):
                    expanded.extend(split_multi_role(s))

                for s in expanded:
                    t = str(s.get("text",""))
                    if not t or _is_numeric_or_phaseish(t):
                        continue
        ...
        for d in toks:
            ...
            keep = True
            for u in dedup:
                if _norm_txt(u["text"]) == dn and _bbox_iou(db, (u["x1"],u["y1"],u["x2"],u["y2"])) >= 0.75:
    ...
    def _read_single_numeric_column_for_rows(self, img, cols, col_key: str, rows_y_spans, *, is_amps: bool,
                                             debug_dir=None, base_name="page"):
        """
        OCR a separated TRIP or POLES column, one row at a time.

        col_key: 'L_TRIP', 'L_POLES', 'R_TRIP', 'R_POLES'
        is_amps: True  -> parse as amps
                 False -> parse as poles

        Returns a list of ints|None, one per row span in rows_y_spans.
        """
        if col_key not in cols:
            return [None] * len(rows_y_spans)

        x1, x2 = map(int, cols[col_key])
        H, W = img.shape[:2]
        ...
        TOP_BAND_FRAC = 0.25
        MIN_CONF = _MIN_COMBO_CONF  # reuse combo floor
        ...
        for row_t in rows_y_spans:
            ...
            def _read(img_small):
                try:
                    return reader.readtext(
                        ...
                        detail=1,
                        paragraph=False,
                        allowlist=_COMBO_ALLOWLIST,  # digits/AP/space/slash
                        text_threshold=0.40,
                        low_text=0.28,
                        mag_ratio=1.0
                    )
            ...
            dets_gry = _read(gry)
            dets_inv = _read(inv)
            dets_raw = dets_gry + dets_inv
            dets_raw = [r for r in dets_raw if float((r[2] or 0.0)) >= MIN_CONF]
            ...
            inv_scale = 1.0 / SCALE
            crop_h = gry.shape[0] if gry is not None else 0

            best_val = None
            best_conf = -1.0
            raw_tokens = []

            for (box, txt, conf) in dets_raw:
                if not txt:
                    continue
                raw_tokens.append({"text": txt, "conf": float(conf)})

                # Convert box coords back to crop space
                scaled_box = [
                    (int(round(x * inv_scale)), int(round(y * inv_scale)))
                    for (x, y) in box
                ]
                ys = [p[1] for p in scaled_box]
                if not ys or crop_h <= 0:
                    continue
                cy = 0.5 * (min(ys) + max(ys))
                cy_norm = cy / float(crop_h)

                # Drop tokens living in the top header-ish band
                if cy_norm < TOP_BAND_FRAC:
                    continue

                # Parse as amps or poles using the same token helpers as combo
                if is_amps:
                    val = self._combo_amp_from_token(txt)
                else:
                    val = self._combo_poles_from_token(txt)
        ...
        # Optional per-column debug JSON
        try:
            if debug_dir:
                os.makedirs(debug_dir, exist_ok=True)
                short_key = col_key.lower()
                self._write_json(
                    os.path.join(debug_dir, f"{base_name.lower()}_{short_key}_strip.json"),
                    {"col_key": col_key, "rows": dbg_rows}
                )
        except Exception:
            pass
    ...
    def parse_from_analyzer(self, analyzer_payload: Dict) -> Dict:
        ...
        # Extend the crop upward by ~10% of the tight header band height to avoid clipping the top of cells
        band_h   = max(1, int(band_y2 - band_y1))
        extra_up = max(2, int(0.25 * band_h))

        y1_dbg = max(0, int(band_y1) - pad_up - extra_up)
        ...
        # Build header bands from a tight band anchored at the header line (stable vs OCR padding)
        tight_h = max(20, int(0.9 * (band_y2 - band_y1)))  # or int(1.2*med_gap)
        hy1 = int(header_y) + 2
        hy2 = min(H - 1, hy1 + tight_h)
        vlines_header = self._detect_vertical_lines(orig_gray, hy1, hy2)
        header_bands  = self._bands_from_vlines(vlines_header, W)

        # === NEW: Save a header-band crop with cell (column) overlays ===
        try:
            if debug_dir and self.debug:
                # Rebuild the exact padded header-band crop that was OCR’d
                header_band_crop = work_gray[y1_dbg:y2_dbg, :]
                cells_overlay = cv2.cvtColor(header_band_crop, cv2.COLOR_GRAY2BGR)
                band_h = cells_overlay.shape[0]

                # Draw each "cell" (i.e., x-band from vertical gridlines) across the header band crop
                # Use a light border and index label for clarity.
                for i, (bx1, bx2) in enumerate(header_bands, start=1):
                    # Guard against degenerate bands
                    x1 = max(0, int(bx1)); x2 = min(cells_overlay.shape[1] - 1, int(bx2))
                    if x2 <= x1:
                        continue
                    # Rectangle for the cell
                    cv2.rectangle(cells_overlay, (x1, 0), (x2, band_h - 1), (0, 180, 255), 2)
                    # Label near the top-left of the cell
                    cv2.putText(
                        cells_overlay,
                        f"C{i}",
                        (x1 + 4, max(14, 4)),
                ...
                # Optionally mark the raw vertical line hits as thin guide lines (helps QA)
                for xl in (vlines_header or []):
                    xl_i = int(xl)
                    if 0 <= xl_i < cells_overlay.shape[1]:
                        cv2.line(cells_overlay, (xl_i, 0), (xl_i, band_h - 1), (80, 80, 80), 1)

                # Write out a dedicated debug artifact for the cells overlay
                cells_path = os.path.join(debug_dir, f"{base_name}_header_band_cells.png")
                cv2.imwrite(cells_path, cells_overlay)
        except Exception as e:
            if self.debug:
                print(f"[PARSER] Failed to write header band cells overlay: {e}")
        ...
        # 2) Run the column strip scan using per-row readers (combo + separated)
        all_finds = {}
        chunk_meta = None  # chunking no longer needed; we scan per-row directly

        # Separated TRIP/POLES columns (only if that side is NOT combined)
        def _read_side_numeric(side_prefix: str):
            trip_key = f"{side_prefix}_TRIP"
            poles_key = f"{side_prefix}_POLES"

            trip_vals = self._read_single_numeric_column_for_rows(
                work_gray, cols_snapped, trip_key, body_row_spans,
                is_amps=True, debug_dir=debug_dir, base_name=base_name
            ) if trip_key in cols_snapped else [None] * len(body_row_spans)

            poles_vals = self._read_single_numeric_column_for_rows(
                work_gray, cols_snapped, poles_key, body_row_spans,
                is_amps=False, debug_dir=debug_dir, base_name=base_name
            ) if poles_key in cols_snapped else [None] * len(body_row_spans)

            return trip_vals, poles_vals

        L_trip_vals: List[Optional[int]] = [None] * len(body_row_spans)
        L_pole_vals: List[Optional[int]] = [None] * len(body_row_spans)
        R_trip_vals: List[Optional[int]] = [None] * len(body_row_spans)
        R_pole_vals: List[Optional[int]] = [None] * len(body_row_spans)

        if not left_combo:
            L_trip_vals, L_pole_vals = _read_side_numeric("L")
        if not right_combo:
            R_trip_vals, R_pole_vals = _read_side_numeric("R")
        ...
        # Optional debug dump roughly matching your old column_data.json
        try:
            col_json_path = os.path.join(os.path.dirname(gridless_out_path), "column_data.json")
            self._write_json(col_json_path, all_finds)
        except Exception as e:
            if self.debug:
                print(f"[PARSER] failed to write column_data.json: {e}")
    ...
    def _columns_from_header_map(self, header_map: Dict[str, List[dict]], page_width: int,
        ...
        combined_right = bool(combined_left)
    ...
    def _ocr_header_band_only(self, gray: np.ndarray, y1: int, y2: int):
        ...
        # Unsharp mask to boost thin header strokes → higher OCR confidence
        _blur = cv2.GaussianBlur(g, (0, 0), 1.0)
        g = cv2.addWeighted(g, 1.6, _blur, -0.6, 0)
        ...
        dets = _pass(g, 1.8) + _pass(inv, 1.8)
        ...
        # Merge overlapping/near-identical detections (keeps highest-conf)
        items = _nms_merge(items, iou_thresh=0.45)
    ...
    def _ocr_combo_cell(self, crop, *, return_dets: bool = False):
        ...
        If return_dets=False:
            returns a flat string of all detected token texts (joined with spaces).
        If return_dets=True:
            returns (text, dets) where:
                text: joined token text (for debug only)
                dets: list of (box, text, conf) in *crop* coordinate space
        """
        if crop is None or crop.size == 0:
        ...
        # Try both polarities and merge
        dets_gry = _read(gry)
        dets_inv = _read(inv)
        dets_raw = dets_gry + dets_inv
        dets_raw = [r for r in dets_raw if float((r[2] or 0.0)) >= _MIN_COMBO_CONF]
        ...
        for (box, txt, conf) in dets_raw:
            ...
            dets_scaled.append((scaled_box, str(txt), float(conf or 0.0)))

        if not dets_scaled:
            return ("", []) if return_dets else ""

        # Joined text is for logging / debug only
        joined_text = " ".join(t for t in texts).upper()
        joined_text = re.sub(r"\s+", " ", joined_text).strip()

        if return_dets:
            return joined_text, dets_scaled
        return joined_text

    def _combo_norm_token(self, txt: str) -> str:
        """
        Normalize OCR token text for combo/separated parsing.
        """
        t = str(txt or "").strip().upper()
        t = t.replace("AMP.", "AMP").replace("AMPS", "AMP")
        t = re.sub(r"\s+", "", t)
        return t

    def _combo_amp_from_token(self, txt: str) -> Optional[int]:
        """
        Return amperage value if this token looks like a valid amp token, else None.
        Accepts:
            - 20, 20A, 20AMP
            - 20/1P, 20A/1P
            - fused forms like 1511 / 15I1 / 1571 / 2013 / 3012 / 2572 / 15011
              (amps = all but last two digits, fake slash = 1/7/I, poles = last digit)
            - compact 3-digit like 152 -> 15A 2P
        """
        t = self._combo_norm_token(txt)
        if not t:
            return None

        # Split on slash first (canonical formats like 20/1P, 20A/1P)
        parts = t.split("/")
        main = parts[0]  # left side is amps candidate

        # Plain amps: 20, 20A, 20AMP
        m = re.fullmatch(r"(\d{1,3})(A|AMP)?", main)
        if m:
            val = int(m.group(1))
            if 5 <= val <= 1200:
                return val

        # General fused numeric-only patterns like:
        #   1511, 3012, 2572, 15011
        # Interpret as: [amps][fake_slash][poles]
        #   amps  = all but last two digits
        #   fake_slash ∈ {1,7,I}
        #   poles ∈ {1,2,3}
        if re.fullmatch(r"\d{4,5}", main):
            amps_str = main[:-2]
            fake     = main[-2]
            poles_ch = main[-1]
            if fake in ("1", "7", "I") and poles_ch in ("1", "2", "3"):
                amps = int(amps_str)
                if 15 <= amps <= 1200 and amps % 5 == 0:
                    return amps

        # Pure compact 3-digit like 152 -> 15A 2P
        m = re.fullmatch(r"(\d{2})([1-3])", main)
        if m:
            amps = int(m.group(1))
            poles = int(m.group(2))
            if 15 <= amps <= 99 and amps % 5 == 0:
                return amps

        return None

    def _combo_poles_from_token(self, txt: str) -> Optional[int]:
        """
        Return pole count if this token looks like a valid poles token, else None.
        Accepts:
            - 1, 2, 3, 1P, 2P, 3P
            - right side of 20A/1P, 20/1P, 20/1
            - 1511 / 15I1 / 1571 / 2013 (fake slash: 1, I, or 7)
            - 152 (compact: 15A 2P -> poles=2)
        """
        t = self._combo_norm_token(txt)
        if not t:
            return None

        # Right side of a slash (20/1P or 20/1)
        parts = t.split("/")
        cand = parts[1] if len(parts) == 2 else t

        # Simple poles: 1, 2, 3, 1P, 2P, 3P
        m = re.fullmatch(r"([1-4])P?", cand)
        if m:
            val = int(m.group(1))
            if 1 <= val <= 4:
                return val
        ...
        #   fake_slash ∈ {1,7,I}
        #   poles ∈ {1,2,3}
        if re.fullmatch(r"\d{4,5}", cand):
            fake     = cand[-2]
            poles_ch = cand[-1]
            if fake in ("1", "7", "I") and poles_ch in ("1", "2", "3"):
                return int(poles_ch)

        # Pure compact 3-digit like 152 -> 15A 2P -> poles=2
        m = re.fullmatch(r"(\d{2})([1-3])", cand)
        if m:
            return int(m.group(2))

        return None

    def _parse_combo_from_detections(self, dets: List[Tuple[List[Tuple[int,int]], str, float]], crop_h: int, raw_text: str = "") -> dict:
        """
        Parse amps/poles from raw EasyOCR detections for a single combo cell.

        dets: list of (box, text, conf) in crop coordinates.
        crop_h: height of the crop in pixels.
        raw_text: optional joined text for debug.
        """
        if not dets or crop_h <= 0:
            return {"amps": None, "poles": None, "raw": raw_text or "", "method": "empty"}

        # Filter by confidence and drop tokens too close to the top of the crop (likely header bleed).
        MIN_CONF = 0.45
        TOP_BAND_FRAC = 0.25  # ignore tokens whose center is in the top 25% of the crop
        ...
        if not candidates:
            return {"amps": None, "poles": None, "raw": raw_text or "", "method": "no_candidates"}

        # Keep only tokens that live below the top band
        candidates = [c for c in candidates if c["cy_norm"] >= TOP_BAND_FRAC]
        if not candidates:
            return {"amps": None, "poles": None, "raw": raw_text or "", "method": "top_only"}

        amps_candidates: List[Tuple[int, float]] = []
        poles_candidates: List[Tuple[int, float]] = []

        for c in candidates:
            txt = c["text"]
            conf = c["conf"]

            a = self._combo_amp_from_token(txt)
            if a is not None:
                amps_candidates.append((a, conf))

            p = self._combo_poles_from_token(txt)
            if p is not None:
                poles_candidates.append((p, conf))

        amps = None
        poles = None

        if amps_candidates:
            amps = max(amps_candidates, key=lambda x: x[1])[0]
        if poles_candidates:
            poles = max(poles_candidates, key=lambda x: x[1])[0]

        if amps is None and poles is None:
            return {"amps": None, "poles": None, "raw": raw_text or "", "method": "no_valid_tokens"}

        return {"amps": amps, "poles": poles, "raw": raw_text or "", "method": "combo_tokens"}
    ...
    def _parse_combo_text(self, s: str) -> dict:
        ...
        # Fused numeric-only patterns like 1511 / 3012 / 15011 / 2572:
        #   amps  = all but last two digits
        #   fake  = second-to-last digit (1,7,I)
        #   poles = last digit (1,2,3)
        if re.fullmatch(r"\d{4,5}", u):
            amps_str = u[:-2]
            fake     = u[-2]
            poles_ch = u[-1]
            if fake in "17I" and poles_ch in "123":
                amps = int(amps_str)
                if 15 <= amps <= 1200 and amps % 5 == 0:
                    return {
                        "amps": amps,
        ...
        # HARD GUARD: if after normalization we still have any alpha group length ≥2,
        # or any letter that is NOT A/P, treat as junk (prevents words from parsing as numbers).
        if re.search(r"[A-Z]{2,}", u) or re.search(r"[B-DF-OQ-RT-Z]", u):
            return {"amps": None, "poles": None, "raw": raw, "method": "badalpha"}

        # SPECIAL-CASE compact 3-digit like 152 -> 15A 2P (do this BEFORE lonely-number guard)
        mc = re.fullmatch(r"(\d{2})([1-3])", u)
        if mc:
            amps = int(mc.group(1))
            poles = int(mc.group(2))
            if 15 <= amps <= 99 and amps % 5 == 0:
                return {"amps": amps, "poles": poles, "raw": raw, "method": "compact_xxp"}

        # NEW: if it's a pure 2–3 digit number and contains no 'P','A','/' -> treat as junk,
        # but the compact_xxp case above has already been handled.
        if re.fullmatch(r"\d{2,3}", u) and not re.search(r"[PA/]", u):
            return {"amps": None, "poles": None, "raw": raw, "method": "lonely_number"}
        ...
        pats = [
            ...
            # Slash-mangled compact like 1511 / 15I1 / 1571 / 2013 / 3012
            r"^(?P<amps>\d{2,3})(?:[17I])(?P<poles>[1-3])$",
    ...
    def read_combo_for_rows(self, img, cols, rows_y_spans, side="L", *, debug_dir=None, base_name="page", header_guard_y: Optional[int] = None):
        ...
        Returns: list of dicts
            [
              {"amps": int|None, "poles": int|None, "raw": str, "method": str},
              ...
            ]
        """
        key = f"{side}_COMBO"
        ...
        for row_t in rows_y_spans:
            ...
            parsed = self._parse_combo_from_detections(dets_here, crop.shape[0], raw_text=raw_txt)
            out.append(parsed)
        ...
        try:
            if debug_dir:
                ...
                # Re-scan rows to get per-row detections (for overlay)
                for row_t in rows_y_spans:
                    r1, r2 = _first2(row_t)
                    if r1 is None or r2 is None:
                        continue
                    ...
                    crop = img[yy1:yy2, xx1:xx2]
                    raw_txt_here, dets_here = self._ocr_combo_cell(crop, return_dets=True)
    ...
    def _scan_tp_column_strips(
        ...
        def read_trip_strip(col_img: np.ndarray):
            ...
            for box, txt, _ in dets:
                ...
                w = x2b - x1b + 1
                h = y2b - y1b + 1
                # Top-edge numeric veto: prevents header bleed like bare "30" at the strip's very top.
                # Only triggers on pure 2–3 digit hits whose bottom sits in the top 20% of this column crop.
                Hc = col_img.shape[0]
                if y2b <= int(0.20 * Hc) and re.fullmatch(r"\d{2,3}", keep):
                    continue
        ...
        for key, band in bands.items():
            ...
            if not suppress_top_wipes:
                ...
                if crop is not None and crop.size and bool(tg_cfg.get("enable", False)):
                    tg = max(
                        int(crop.shape[0] * float(tg_cfg.get("frac", 0.05))),
                        int(tg_cfg.get("min_px", 0))
                    )
                    if tg > 0:
                        crop[:tg, :] = 255
