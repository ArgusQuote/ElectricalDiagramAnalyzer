# OcrLibrary/PanelHeaderParserV12.py
...
class PanelParser:
    ...
    _NAME_STOPWORDS = {
        "NEW","EX","EXISTING","TYPICAL","TYPE","SYSTEM","DISTRIBUTION","NORMAL","EMERGENCY",
    ...
    def parse_panel(
        ...
        # --------------------------------------------------
        # NAME + EXISTING-PANEL CLASSIFICATION
        #
        # Important:
        # Capture the raw selected NAME value and its closest raw NAME
        # label BEFORE normalizing/cleaning the final panel name.
        # --------------------------------------------------
        name_pick = chosen_map.get("NAME")

        raw_name_value = str(
            name_pick.get("text") if name_pick else ""
        ).strip()

        raw_name_label = ""

        # labels_map["NAME"] already contains only text that the parser
        # classified as a NAME label. Find the label closest to the
        # selected NAME value so unrelated labels cannot affect this panel.
        name_labels = labels_map.get("NAME") or []

        if name_pick and name_labels:
            try:
                name_x = float(name_pick.get("xc", 0.0))
                name_y = float(name_pick.get("yc", 0.0))

                closest_name_label = min(
                    name_labels,
                    key=lambda lbl: (
                        (float(lbl.get("xc", 0.0)) - name_x) ** 2
                        + (float(lbl.get("yc", 0.0)) - name_y) ** 2
                    ),
                )

                raw_name_label = str(
                    closest_name_label.get("text") or ""
                ).strip()

            except Exception:
                raw_name_label = ""

        # Existing classification happens against the untouched NAME
        # label/value only.
        existing_info = self._detect_existing_panel(
            raw_name_label=raw_name_label,
            raw_name_value=raw_name_value,
        )
        ...
        # Only now do we normalize the user-facing/internal panel name.
        name = self._normalize_name_output(
            raw_name_value
        )
        ...
        if self.debug:
            print(
                ...
                f"raw_label={raw_name_label!r} "
                f"raw_value={raw_name_value!r} "
                f"final_name={name!r} "
                f"flag={existing_info.get('suspectedExisting', False)!r} "
                f"detection={existing_info.get('existingDetection')!r}"
            )
        ...
        result = {
            ...
            # Detection never automatically skips the panel.
            "suspectedExisting": bool(
                existing_info.get("suspectedExisting", False)
            ),
            "existingReason": str(
                existing_info.get("existingReason") or ""
            ),
            "existingDetection": str(
                existing_info.get("existingDetection") or ""
            ),
        }
    ...
    def _detect_existing_panel(
        self,
        raw_name_label: str,
        raw_name_value: str,
    ) -> dict:
        """
        Classify possible existing equipment using ONLY the raw NAME
        label and raw NAME value.

        Important:
        We intentionally do NOT scan the rest of the panel header.

        EXISTING detection in the NAME label is fuzzy enough to tolerate
        small OCR errors such as:
            EXISTING
            EEXISTING
            EXIST1NG
            IEXIST1NG
        ...
        Maximum allowed edit distance from EXISTING: 2.

        E- naming convention remains a suspected-existing rule on the
        NAME value only.

        Metadata only. Never skips or excludes the panel.
        """

        label_up = re.sub(
            r"\s+",
            " ",
            str(raw_name_label or "").upper(),
        ).strip()

        value_up = re.sub(
            r"\s+",
            " ",
            str(raw_name_value or "").upper(),
        ).strip()

        # --------------------------------------------------
        # Small Levenshtein/edit-distance helper.
        ...
        # --------------------------------------------------
        # 1) Fuzzy EXISTING detection in NAME LABEL only.
        #
        # Break the label into words and compare each word
        # against EXISTING.
        #
        # Examples:
        #   Existing Branch Panel:
        #   EExisting Branch Panel:
        #   EXIST1NG PANEL:
        #   IEXIST1NG PANEL:
        # --------------------------------------------------
        ...
        for word in label_words:
            ...
            # Keep the candidate roughly the same size as EXISTING.
            # This prevents comparing every random short/long word.
            ...
            if distance <= 2:
                return {
                    "suspectedExisting": True,
                    "existingReason": (
                        f"Panel name label appears to identify the panel "
                        f"as existing: '{raw_name_label}' "
                        f"(matched '{word}', edit distance {distance})."
                    ),
                    "existingDetection": "explicit_existing",
                }

        # --------------------------------------------------
        # 2) Explicit EX abbreviation in NAME LABEL only.
        #
        # Examples:
        #   EX PANEL:
        #   EX. PANEL:
        ...
        # --------------------------------------------------
        if re.search(
            r"\bEX\.?\s+(?:BRANCH\s+)?(?:PANELBOARD|PANEL|PNL)\b",
            label_up,
        ):
            return {
                "suspectedExisting": True,
                "existingReason": (
                    f"Panel name label uses an EX existing-panel designation: "
                    f"'{raw_name_label}'."
                ),
                "existingDetection": "explicit_ex",
            }

        # --------------------------------------------------
        # 3) Leading (E) existing-equipment designation
        #
        ...
        #       (E)L6EH
        #
        # or fused into the NAME label/value OCR token:
        #       BRANCH PANEL: (E)L6EH
        #
        # We still inspect NAME context only.
        # --------------------------------------------------

        # Normal separated NAME value:
        #   raw_name_value = "(E)L6EH"
        if re.match(
            r"^\(\s*E\s*\)\s*[-:]?\s*[A-Z0-9]",
            value_up,
        ):
            return {
                "suspectedExisting": True,
                "existingReason": (
                    f"Panel designation '{raw_name_value}' begins with '(E)'."
                ),
                "existingDetection": "e_parenthetical_prefix",
            }

        # Fused NAME label/value:
        #   raw_name_label = "BRANCH PANEL: (E)L6EH"
        if re.search(
            r":\s*\(\s*E\s*\)\s*[-:]?\s*[A-Z0-9]",
            label_up,
        ):
            return {
                "suspectedExisting": True,
                "existingReason": (
                    f"Panel name field contains an '(E)' existing-panel "
                    f"designation: '{raw_name_label}'."
                ),
                "existingDetection": "e_parenthetical_prefix",
            }

        # --------------------------------------------------
        # 4) E- naming convention in RAW NAME VALUE only.
        #
        # Matches:
        #   E-2A
        #   E-LP1
        ...
        if re.match(
            ...
            return {
                ...
                "existingDetection": "e_prefix",
            }

        return {
            "suspectedExisting": False,
            "existingReason": "",
            "existingDetection": "",
        }
