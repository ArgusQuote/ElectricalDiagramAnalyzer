...
class PanelboardEngine(BaseEngine):
    ...
    def _find_main_breaker_config_params(
        self,
        family: str,
        builder,
        amps: int,
        ...
        """
        Re-resolve the final main-breaker panelboard configuration after a bump.

        This prevents using stale frame/kit/barrier metadata from the original
        candidate when the successful panel was actually built from bumped
        amps/spaces/material values.
        """
        family = str(family or "").upper()
        amps = _safe_int(amps, 0)
        spaces = _safe_int(spaces, 0)
        lookup_voltage = _safe_int(lookup_voltage, 0)
        ...
        for key, params in table.items():
            # NQ/NF main breaker keys:
            # (minA, maxA, spaces, phase, voltageSpec)
            minA, maxA, spc, phs, voltSpec = key

            volts = voltSpec if isinstance(voltSpec, (list, tuple)) else (voltSpec,)

            if not (minA <= amps <= maxA):
                continue
            if spc != spaces:
                continue
            if phs != phase:
                continue
            if lookup_voltage not in volts:
                continue

            matches.append((maxA, params))

        if not matches:
            return None

        # Prefer the tightest amp bucket.
        ...
        for mode, try_amps, try_spaces, try_material in attempts:
            ...
            if not isinstance(out, str):
                return out, {
                    ...
                    "mainBreakerType": attrs.get("mainBreakerType"),
                }
...
#            return {"error": "Missing interrupting rating for main breaker"}
        ...
        for fam in family_try_order:
            ...
            # The panel may have been bumped to a different valid config.
            # Re-resolve final params so kits/frame/barrier come from the final config,
            # not the original pre-bump candidate.
            resolved_final = self._find_main_breaker_config_params(
                family=fam,
                builder=(nq_builder if fam == "NQ" else nf_builder),
                amps=bump_meta["amperage"],
                spaces=bump_meta["spaces"],
                phase=phase,
                lookup_voltage=lookup_voltage,
            )
            ...
            if resolved_final is None:
                last_family_error = (
                    f"{fam} panel built after bump, but final configuration metadata "
                    f"could not be re-resolved for {bump_meta['amperage']}A / "
                    f"{bump_meta['spaces']} spaces / {phase} / {lookup_voltage}V."
                )
                panel_output = None
                bump_meta = None
                selected_family = None
            ...
            _final_maxA, final_params = resolved_final

            selected_params = final_params

            # Preserve the actual frame used by the successful build when possible.
            final_frame_code = bump_meta.get("mainBreakerType") or trial_full_frame_code

            # Safety: if the preserved frame is not valid for the final config,
            # fall back to the first allowed frame from the final config.
            if final_frame_code not in final_params[2]:
                final_frame_code = final_params[2][0]

            full_frame_code = final_frame_code
            all_kits = list(final_params[3:-1])
            barrier_kit = final_params[-1]
        ...
        if snapped_ir is None:
            panel_output.setdefault("Notes", []).append(
                f"Main breaker frame {rating_key} cannot satisfy requested {raw_ir}kAIC at {rating_voltage}V. "
                "Main breaker requires user review. Branch breakers were still generated where possible."
            )
            panel_output["Main Breaker Error"] = (
                f"No valid interrupting rating for frame {rating_key} at {rating_voltage}V "
                f"that meets or exceeds {raw_ir}kAIC."
            )

            # Do not build a fake/underrated main breaker.
            # Finish the rest of the reviewable BOM.
            if panelAttrs.get("_resize_note"):
                panel_output.setdefault("Notes", []).append(panelAttrs["_resize_note"])

            # Clean up hidden metadata before branch breaker build/output.
        ...
        if isinstance(breaker_part, str) and breaker_part.startswith("Invalid interrupting rating"):
            ...
            if next_ir is None:
                panel_output.setdefault("Notes", []).append(
                    f"Main breaker frame {rating_key} could not be bumped to a valid interrupting rating "
                    f"at or above {breaker_attrs['intRating']}kAIC. User review required."
                )
            ...
            for two_letter in frames:
                ...
                if ir is None:
                    result.setdefault("Notes", []).append(
                        f"I-Line main breaker frame {frame_letter} cannot satisfy requested "
                        f"{desired_ir}kAIC at {rating_voltage}V. Trying next frame."
                    )
                    continue
                ...
                if isinstance(part, str):
                    ...
                    if isinstance(part, str) and part.startswith("Invalid interrupting rating"):
                        ...
                        if bumped_ir is None:
                            result.setdefault("Notes", []).append(
                                f"I-Line main breaker frame {frame_letter} could not be bumped to a valid "
                                f"interrupting rating above {breaker_attrs['intRating']}kAIC at {rating_voltage}V."
                            )
                            continue
    ...
    def _build_nq_branch_breakers(self, panel_result: dict, raw: dict, detected_breakers: list):
        ...
        for br in detected_breakers:
            ...
            if ir_ladder:
                ir = next((k for k in sorted(ir_ladder) if k >= desired_ir), None)
                if ir is None:
                    panel_result.setdefault("Notes", []).append(
                        f"Branch {poles}P@{raw_amps}A using {base} cannot satisfy requested "
                        f"{desired_ir}kAIC at {rating_voltage}V. User review required."
                    )
                    continue
            else:
                panel_result.setdefault("Notes", []).append(
                    f"No interrupting rating ladder found for {base} at {rating_voltage}V. "
                    f"Branch {poles}P@{raw_amps}A requires user review."
                )
                continue
        ...
        def _try_build_branch(mccb_builder, attrs, frame_letter, rating_voltage):
            ...
            def _attempt(a):
                ...
                if isinstance(out, str) and out.lower().startswith("invalid interrupting rating"):
                    next_ir = snap_int_rating(frame_letter, rating_voltage, int(a["intRating"]) + 1)

                    if next_ir is None:
                        return (
                            f"Invalid interrupting rating: frame {frame_letter} cannot satisfy "
                            f"{a['intRating']}kAIC or higher at {rating_voltage}V"
                        )
            ...
            attrs2 = attrs.copy()
            next_ir = snap_int_rating(frame_letter, rating_voltage, int(attrs2["intRating"]) + 1)

            if next_ir is None:
                return (
                    f"Invalid interrupting rating: frame {frame_letter} cannot satisfy "
                    f"{attrs2['intRating']}kAIC or higher at {rating_voltage}V"
                )

            attrs2["intRating"] = next_ir
            ...
            while placed_units < count:
                ...
                for frame in order:
                    ...
                    if rating_type == "SERIES_RATED":
                        ...
                        ir = valid_ir[0] if valid_ir else None
                    else:
...
def snap_int_rating(frame: str, voltage: int, desired: int) -> int | None:
    """
    Return the next valid interrupting rating at or above the requested value.

    Important:
    - Exact match is allowed.
    - Upward bump is allowed.
    - Downward bump is NOT allowed.
    - If the frame cannot satisfy the requested AIC, return None.
    """
    try:
        desired = int(desired)
    except Exception:
    ...
    frame = str(frame or "").upper()
    voltage = _safe_int(voltage, 0)
