...
# ---------- IO PATHS ----------
INPUT_PDF = Path("~/ElectricalDiagramAnalyzer/DevEnv/SourcePdf/S5.pdf").expanduser()
...
def score_page_for_panelboard_relevance(page_dict: dict) -> float:
    ...
    # ------------------------------------------------------------
    # Strong positives: actual 26 24 16 section header, not references
    # ------------------------------------------------------------
    ...
    if any(re.search(r"\blow\s+voltage\s+panelboards\b", r) for r in top_regions):
        score += 16.0
    elif any(re.search(r"\bpanelboards\b", r) for r in top_regions):
        score += 6.0

    if re.search(r"\b26\s+24\s+16\s*[-–—]\s*1\b", norm_full):
        score += 20.0

    # ------------------------------------------------------------
    # Body/support terms (light weight only)
    # ------------------------------------------------------------
    ...
    # ------------------------------------------------------------
    # Penalties for reference / TOC / related-work pages
    # ------------------------------------------------------------
    ...
    # Pages listing many other section numbers are usually not the true section
    ...
    # Specific penalty for subsection references like 26 24 16.13 / 26 24 16.16
    if re.search(r"\b26\s+24\s+16\.\d+\b", norm_full):
        score -= 12.0
...
def get_panelboard_section_identity(page_dict: dict) -> Optional[str]:
    """
    Return the best exact panelboard section identity found on a page.
    Priority is exact subsection match first, then parent section.
    """
    candidate_regions = [
        page_dict["regions"]["top_center"],
        page_dict["regions"]["top_left"],
...
def find_panelboard_section_candidates(pages: List[dict]) -> List[dict]:
    """
    Find pages that are actual panelboard section headers and label them
    with their specific section identity.
    """
    candidates = []
...
def choose_best_panelboard_candidate(candidates: List[dict]) -> Optional[dict]:
    """
    Choose the best candidate by business priority first, then header score,
    then earlier page if still tied.
    """
    if not candidates:
        return None
...
def find_best_start_page(pages: List[dict]) -> Tuple[Optional[int], float, str]:
    ...
    for idx, page in enumerate(pages):
        ...
        has_panel_title = (
            any("panelboards" in r for r in top_regions)
            or "low voltage panelboards" in full_text
        )
...
def refine_to_section_start(pages: List[dict], best_idx: int) -> int:
    """
    Find the best actual header page in the lookback window using stricter header rules.
    """
    ...
    for idx in range(lookback, best_idx + 1):
        ...
        has_panelboard_title = (
            any("panelboards" in r for r in top_regions)
            or "low voltage panelboards" in full_text
        )

        # Reject subsection references like 26 24 16.13 / 26 24 16.16
        is_subsection_reference_only = re.search(r"\b26\s+24\s+16\.\d+\b", full_text) is not None

        if has_true_section_header and has_panelboard_title and not is_subsection_reference_only:
...
def collect_multi_page_section(
    ...
    for idx in range(start_idx, len(pages)):
        ...
        if idx > start_idx:
            # If targeting a subsection like 26 24 16.13 or 26 24 16.16,
            # stop at the next panelboard sibling section header.
            ...
            # Hard stop for next major section family
            if (
            ...
            # Generic stop at next Division 26 section only if the TOP of the page
            # looks like a real new section header, not just a body reference.
...
def detect_bussing_material(text: str, page: Optional[int], source: str) -> DetectedValue:
    # ------------------------------------------------------------
    # Pass 1: raw-line list handling for formats like:
    # Panelboard Buses:
    #   1. Copper
    # ------------------------------------------------------------
    lines = [ln.strip() for ln in normalize_preserve_lines(text).splitlines()]
    ...
    bus_header_patterns = [
        re.compile(r"^(?:[A-Z]\.\s*|\d+[\.\)]\s*)?panelboard\s+buses\s*:?", re.IGNORECASE),
        re.compile(r"^(?:[A-Z]\.\s*|\d+[\.\)]\s*)?buses\s*:?", re.IGNORECASE),
        re.compile(r"^(?:[A-Z]\.\s*|\d+[\.\)]\s*)?bus\s*:?", re.IGNORECASE),
        re.compile(r"^(?:[A-Z]\.\s*|\d+[\.\)]\s*)?bus\s+material\s*:?", re.IGNORECASE),
        re.compile(r"^(?:[A-Z]\.\s*|\d+[\.\)]\s*)?main\s+bus\s*:?", re.IGNORECASE),
        re.compile(r"^(?:[A-Z]\.\s*|\d+[\.\)]\s*)?phase\s+bus\s*:?", re.IGNORECASE),
    ]

    for i, line in enumerate(lines):
        if any(p.search(line) for p in bus_header_patterns):
            lookahead = lines[i:i+10]
            block_text = "\n".join(lookahead)
            norm_block = normalize_for_matching(block_text)

            if re.search(r"\bcopper\b", norm_block):
                return DetectedValue(
                    value="COPPER",
                    matched_phrase="panelboard buses list indicates copper",
                    evidence=[_make_evidence(page, source, block_text[:1200])]
                )

            if re.search(r"\baluminum\b|\baluminium\b", norm_block):
                return DetectedValue(
                    value="ALUMINUM",
                    matched_phrase="panelboard buses list indicates aluminum",
                    evidence=[_make_evidence(page, source, block_text[:1200])]
                )

    # ------------------------------------------------------------
    # Pass 2: sentence/chunk logic for inline wording
    # ------------------------------------------------------------
    chunks = sentenceish_chunks(text)

    copper_hits = []
    aluminum_hits = []
    bus_terms = ["bus", "buss", "bussing", "buses", "bus bars", "busbar", "busbars"]

    for chunk in chunks:
        if _token_near_token(chunk, ["copper", "cu"], bus_terms, max_gap=8):
            copper_hits.append(chunk)

        if _token_near_token(chunk, ["aluminum", "aluminium", "al"], bus_terms, max_gap=8):
            aluminum_hits.append(chunk)

        if _contains_any_normalized(chunk, [
            "copper bus",
            "copper buss",
            "copper bussing",
            "copper bus bars",
...
def analyze_pdf_panelboard_specs(pdf_path: str) -> SpecAnalysisResult:
    ...
    if not chosen_candidate:
        ...
        # fallback to old logic if no exact header candidates found
